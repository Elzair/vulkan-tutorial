...
private:
  ...
  void createVertexBuffer(  )
  {
    ...
    if ( vkCreateBuffer( this->device, &bufferInfo,
                         nullptr, &this->vertexBuffer ) != VK_SUCCESS )
    {
    ...
    if ( vkAllocateMemory( this->device, &allocInfo, nullptr, &vertexBufferMemory ) != VK_SUCCESS )
    {
      throw std::runtime_error( "Failed to allocate vertex buffer memory!" );
    }

    vkBindBufferMemory( this->device, this->vertexBuffer,
                        vertexBufferMemory, 0 );

    // Memory map Vertex Buffer
    void* data;
    vkMapMemory( this->device, vertexBufferMemory, 0, bufferInfo.size, 0, &data );
