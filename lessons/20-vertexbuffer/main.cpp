...
private:
  ...
  void createVertexBuffer(  )
  {
    ...
    if ( vkCreateBuffer( this->device, &buffer_info,
                         nullptr, &this->vertex_buffer ) != VK_SUCCESS )
    {
    ...
    vkGetBufferMemoryRequirements( this->device, this->vertex_buffer, &memreqs );

    VkMemoryAllocateInfo alloc_info = {};
    alloc_info.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO;
    alloc_info.allocationSize = memreqs.size;
    alloc_info.memoryTypeIndex = findMemoryType( this->physical,
                                                 memreqs.memoryTypeBits,
                                                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
                                                 VK_MEMORY_PROPERTY_HOST_COHERENT_BIT );

    if ( vkAllocateMemory( this->device, &alloc_info, nullptr, &vertex_buffer_memory ) != VK_SUCCESS )
    {
      throw std::runtime_error( "Failed to allocate vertex buffer memory!" );
    }

    vkBindBufferMemory( this->device, this->vertex_buffer,
                        vertex_buffer_memory, 0 );

    // Memory map Vertex Buffer
    void* data;
    vkMapMemory( this->device, vertex_buffer_memory, 0, buffer_info.size, 0, &data );
    std::memcpy( data, vertices.data(), (size_t)buffer_info.size );
    vkUnmapMemory( this->device, vertex_buffer_memory );
  }
